...
def client():
    ...
    _app.config['WTF_CSRF_ENABLED'] = False  # Disable CSRF for easier testing
    _app.config['SECRET_KEY'] = 'test-secret-key'
    _app.config['SERVER_NAME'] = 'localhost'  # Needed for URL building in tests
    ...
    if hasattr(_app, 'limiter') and _app.limiter:
        # Completely disable the limiter
        _app.limiter.enabled = False

        # Clear all rate limit storage
        try:
            if hasattr(_app.limiter, '_storage') and _app.limiter._storage:
                if hasattr(_app.limiter._storage, 'reset'):
                    _app.limiter._storage.reset()
                if hasattr(_app.limiter._storage, 'clear'):
                    _app.limiter._storage.clear()
                # Try to clear all keys in memory storage
                if hasattr(_app.limiter._storage, 'storage'):
                    _app.limiter._storage.storage.clear()
        except:
            pass

        # Override the limit decorator to be a no-op
        original_limit = _app.limiter.limit
        ...
        _app.limiter.limit = noop_limit

        # Override check to always allow
        _app.limiter.check = lambda *args, **kwargs: True

        # Also set default_limits to empty
        _app.limiter.default_limits = []
    ...
    with _app.test_client() as client:
        with _app.app_context():
            # Create all tables
            db.create_all()
            # Set store as open for tests (default date is future; tests need store open)
            AppSetting.set('store_open_date', '2020-01-01')
            db.session.commit()
...
@pytest.fixture
def app(client):
    """Return the configured Flask app (DB already set up by client fixture)."""
    return _app


@pytest.fixture
def db_session(client):
    """Return the active SQLAlchemy session (shares DB with client fixture)."""
    return db.session
