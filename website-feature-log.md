...
Internal admin tool ("matching game") to resolve the backlog of re-photographed items listed under the internal Campus Swap account that actually belong to real sellers. Page: `GET /admin/warehouse/rephoto/report` (`_has_ops_access()`), reached via a "Match to Sellers" button on the Re-Photograph Items page. Shows a dense clickable grid of rephotographed items (those with a campaign-era `ItemPhoto.captured_at`), defaulting to Campus-Swap-owned items with a toggle for all. Each card shows the warehouse re-photo (`InventoryItem.rephoto_photo_url`), item #, and title. Clicking a card opens a modal to: edit the title, search + assign the real seller (`/admin/warehouse/seller-search`), pick which of that seller's original listings this replaces (`/admin/warehouse/seller-items`, shown as `#id — title (status)`), and set L×W×H dimensions. Saving (`POST /admin/warehouse/rephoto/match/<id>`) reassigns the item to the seller, applies the title/dimensions, and — if an original was chosen — hides that original from the shop (`ai_approved=False`) and links it via the new `InventoryItem.replaced_by_item_id` (migration `454f7f6bc046`). The matched card is removed live and drops off the backlog. No buyer-facing shop-query changes. Deferred: AI description re-run (separate, post-prompt-change) and seller-dashboard grouping of replaced items.
