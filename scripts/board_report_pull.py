...
def section_10_truth():
    ...
    # The mattress unit is now itemised in the database — the 37 identical twins were
    # expanded into individual rows and priced. Mattresses that still carry no price are
    # valued at the flat rate below (real prices are never overridden). Change this one
    # number to revalue them.
    MATTRESS_UNIT = 'Unit 119'
    MATTRESS_GENERIC_PRICE = 100.0
    ...
    # ── mattress unit: real prices where set, the flat rate where not ───────
    ...
    mattress_items = len(m_rows)
    m_priced = [i for i in m_rows if i.price and float(i.price) > 0]
    m_unpriced = mattress_items - len(m_priced)
    mattress_value = round(sum(float(i.price) for i in m_priced)
                           + m_unpriced * MATTRESS_GENERIC_PRICE, 2)
    m_row_ids = {i.id for i in m_rows}
    m_twins = sum(1 for i in m_rows if i.stock_group_id)
    ...
    priced_value = float(db.session.query(func.sum(InventoryItem.price)).filter(
        InventoryItem.id.in_(shop_ids)).scalar() or 0)
    # Unpriced mattresses carry the flat rate; everything else unpriced is excluded.
    estimated_value = round(m_unpriced * MATTRESS_GENERIC_PRICE, 2)
    list_value = priced_value + estimated_value
    priced_items = len(shop_ids) + m_unpriced
    ...
    # A multi-unit listing expanded into clones copies its campaign photos to every
    # clone. The crew shot the original once, so clones are excluded here to keep this
    # the count of photographs actually taken.
    clone_ids = [r[0] for r in db.session.query(InventoryItem.id).filter(
        InventoryItem.stock_group_id.isnot(None),
        InventoryItem.id.notin_(
            db.session.query(func.min(InventoryItem.id)).filter(
                InventoryItem.stock_group_id.isnot(None)
            ).group_by(InventoryItem.stock_group_id)))]

    def camp(*extra):
        return InventoryItem.query.filter(
            InventoryItem.id.in_(campaign),
            InventoryItem.id.notin_(clone_ids or [-1]), *extra)
    ...
    for name, label in REAL_UNIT_LABELS.items():
        ...
        if name == MATTRESS_UNIT:
            ...
            m['avg_price'] = round(mattress_value / mattress_items, 2)
    ...
    # Mattresses valued at the flat rate (no price of their own) land in one band.
    for band, lo, hi in bands:
        if lo <= MATTRESS_GENERIC_PRICE < hi:
            next(b for b in price_bands if b['band'] == band)['count'] += m_unpriced
            break
    ...
    if m_unpriced:
        m_cat = next((i.category.name for i in m_rows if i.category), 'Bedroom')
        row = next((c for c in live_by_category if c['category_name'] == m_cat), None)
        est_value = m_unpriced * MATTRESS_GENERIC_PRICE
        if row:
            row['count'] += m_unpriced
            row['list_value'] += est_value
            row['avg_price'] = round(row['list_value'] / row['count'], 0)
        else:
            live_by_category.append({'category_name': m_cat, 'count': m_unpriced,
                                     'avg_price': MATTRESS_GENERIC_PRICE,
                                     'list_value': est_value})
    ...
    payload = {
        ...
        'mattress_priced_rows': len(m_priced),
        'mattress_estimated_rows': m_unpriced,
        'mattress_twins': m_twins,
