...
ai_photo_enhanced (Boolean, default False, server_default='0') — True once OpenAI background-replacement photo enhancement succeeds for this item. On success the pre-enhancement original cover is preserved into gallery_photos and photo_url is rewritten to an `ai_enhanced_*` file.
InventoryItem.original_photo_url (property, not a column) — the seller's ORIGINAL photo: photo_url if it isn't an `ai_enhanced_*` file, else the first non-AI gallery photo, else the cover. Used by the warehouse Route Photo Reports so a physical audit sees the real item, not the AI-retouched background.
InventoryItem.rephoto_photo_url (property, not a column) — the warehouse re-photography shot: first campaign gallery photo (`ItemPhoto.captured_at` set), front view preferred, else `original_photo_url`. Used by the rephoto matching report.
InventoryItem.display_cover_url (property, not a column) — cover for admin/internal thumbnails: `photo_url` if set, else the first `visible_gallery_photos` (or `gallery_photos`) photo, else None. Display-only fallback so rephoto / quick-capture items (which keep `photo_url` NULL by design) still show a thumbnail instead of the empty placeholder. NEVER writes/backfills the cover — respects the never-touch-cover constraint and doesn't collide with the deferred post-process cover-promotion pass. Used by the item detail drawer (`item_detail_partial.html` builds its gallery with the same NULL-cover→gallery fallback), the QC needs-info queue, warehouse search rows, rephoto search rows, and storage-audit collapsed rows.
replaced_by_item_id (Integer FK → inventory_item, nullable) — set on a seller's ORIGINAL listing when a rephotographed item replaces it (via the matching report). The original is also set `ai_approved=False` to drop it from the shop; this link is for grouping/undo and marking it "replaced" rather than merely unapproved. Migration 454f7f6bc046.
...
rephoto_disposition (String 20, nullable) — how a Campus-Swap-owned rephoto item was cleared from the matching backlog: NULL = still in backlog | 'discarded' (junk/duplicate — also barred from the shop) | 'kept' (Campus Swap keeps & lists it). Set by admin_rephoto_dispose. Migration 46f98d884eeb.
stock_quantity (Integer, default 1, server_default='1') — DESIRED unit count for a multi-unit "identical stock" listing (e.g. 37 matching dressers). Drives a one-time clone-expansion on publish, after which each row carries stock_quantity=1. Migration c2554b94906c.
stock_group_id (String 36, nullable, indexed) — ties the cloned one-unit rows of a multi-unit listing together (Design B). NULL = ordinary single item. Migration c2554b94906c.
mattress_size (String 20, nullable) — one of MATTRESS_SIZES keys (twin|twin_xl|full|queen|king|cal_king). When set, the shop shows the nominal size name alongside numeric dimensions. Set (and used to autofill canonical L/W) via _apply_mattress_size_from_form. Migration 700635cb195e.
InventoryItem.original_photo_urls (property, not a column) — ordered list of the seller's ORIGINAL (pre-AI, not `ai_enhanced_`-prefixed) photo filenames: cover first if original, then gallery by (sort_order, id), de-duped. First element equals original_photo_url. Backs the Route Photo Report lightbox carousel (emitted as a data-photos JSON attribute on each card).
InventoryItem.stock_available_count (property, not a column) — units of this listing currently for sale: 1 for an ordinary item, else count of still-available rows sharing stock_group_id. Shown as "N available" on the product page + shop card.
InventoryItem.mattress_size_label (property, not a column) — human label for mattress_size (e.g. 'Queen'), or None.
```

**Module constant `MATTRESS_SIZES` (models.py):** canonical map keyed by size → {label, length, width} (inches). Single source of truth for the entry-dropdown autofill and the shop's nominal-size display. Keys: twin, twin_xl, full, queen, king, cal_king.
...
| `GET /inventory` | `inventory` | Shop front, category filter + search. Excludes `rephoto_disposition='discarded'` (NULL-safe). Collapses multi-unit stock groups to ONE representative card (lowest-id available unit) via a `NOT EXISTS` filter; card shows "N available" (`stock_available_count`). |
...
| Route | Function | Notes |
|---|---|---|
| `POST /cart/add/<id>` | `cart_add` | Add item to active cart (user or guest session token). For a multi-unit stock listing, `_pick_available_stock_unit` swaps the clicked representative for a free available unit in the group (not already in this cart, not held), so concurrent buyers get distinct units. |
...
| `GET /admin/warehouse/routes/photo-report` | `admin_warehouse_route_photo_report_all` | Standalone printable photo report covering EVERY route on one page — one compact block per route (most-recent-first, same shift set as the chip list), each a dense 7-wide grid of all its items in stop order (no per-stop headers, no amber notice). Routes pack onto shared rows. Read-only. `_has_warehouse_access()`. **`?unmatched=1` toggle ("Not yet matched"):** keeps the per-route layout but shows only items NOT yet through a rephoto match — excludes BOTH the superseded original (`replaced_by_item_id` set) AND the replacement item (any row that is the target of another row's `replaced_by_item_id`, via a `NOT IN replacement_target_ids` subquery). Routes/stops with nothing unmatched drop out. Photos open a carousel (see lightbox partial). |
| `GET /admin/warehouse/routes/<int:shift_id>/photo-report` | `admin_warehouse_route_photo_report` | Standalone printable photo grid of every item on ONE route, grouped by stop. Read-only, no status filter (matches Route Browse). Ordered stops by `ShiftPickup.stop_order`, NULL stops last (alpha by seller) under "Unordered stops" + amber notice. Sellers with zero items omitted. 404 if shift missing. `_has_warehouse_access()`. Both routes share the `_build_route_photo_report(shift, unmatched_only=False)` helper. |
| `POST /admin/warehouse/bulk-move` | `admin_warehouse_bulk_move` | Bulk-move items between storage units. _has_ops_access(). |
| `GET /admin/warehouse/seller-search` | `admin_warehouse_seller_search` | JSON live seller search (name/email, min 2 chars, 15 rows) used by Log Item and rephoto seller pickers. Each row now includes `is_internal` (True for the Campus Swap account) so the match modal can treat picking Campus Swap as a revert. `_has_warehouse_access()`. |
...
| `GET /admin/warehouse/rephoto/report` | `admin_rephoto_match_report` | Rephoto **matching** report. Clickable grid of rephotographed items (`_rephoto_reshot_item_ids`); `?scope=internal` (default — Campus-Swap-owned backlog, excludes rejected/sold) or `?scope=all`. Shows `item.rephoto_photo_url`. Backlog (scope=internal) now also excludes dispositioned items (`rephoto_disposition IS NULL`); scope=all still shows discarded/kept items with a badge. Passes `mattress_sizes=MATTRESS_SIZES` for the size dropdown. `_has_ops_access()`. |
| `GET /admin/warehouse/seller-items` | `admin_warehouse_seller_items` | JSON: a seller's own listings (`?seller_id=`) for the "original to replace" dropdown — `[{id,title,status}]`, excludes rejected + already-`replaced_by` items. `_has_ops_access()`. |
| `POST /admin/warehouse/rephoto/match/<int:item_id>` | `admin_rephoto_match_save` | Assign rephotographed item (B) to a real seller + set title/dimensions/mattress size; optional `original_item_id` (must belong to the seller) → sets that original's `ai_approved=False` (hides from shop) and `replaced_by_item_id=B.id`. On publish, expands stock if `stock_quantity>1`. **Picking the Campus Swap (internal) account is no longer an error** — it performs a revert (`_revert_rephoto_item_to_campus_swap`) instead. Fetch endpoint, JSON. `_has_ops_access()`. |
| `POST /admin/warehouse/rephoto/dispose/<int:item_id>` | `admin_rephoto_dispose` | Clear a Campus-Swap-owned rephoto item from the matching backlog. `action=` one of: `discard` (junk/duplicate → `rephoto_disposition='discarded'`, barred from shop), `keep` (Campus Swap keeps & lists → requires storage; dims optional; sets `rephoto_disposition='kept'`, saves quantity/mattress size, publishes now if ready else auto-lists on AI approval), `restore` (undo discard/keep → disposition NULL, un-publishes a kept item back to `pending_valuation`), `revert` (undo a MATCH → move a real-seller item back to Campus Swap via `_revert_rephoto_item_to_campus_swap`). Fetch endpoint, JSON. `_has_ops_access()`. |
...
**Pickup vs delivery orders.** A sold item is a *buyer pickup* — never routed onto a truck — when either (a) the buyer used the pickup code at checkout (`Order.promo_code` is set) or (b) we marked it sold by hand, which creates no `BuyerOrder` at all.

The pickup code means "I am collecting this myself", **not** "waive my delivery fee". At `/checkout/delivery` the buyer can enter it instead of an address: `_pickup_pending_payload()` stamps the warehouse (`_warehouse_address_parts()`) as the destination with `distance_miles=0`, `zone=None`, `zone_fee=0`, so the radius check never runs — that check was blocking buyers willing to drive further than we deliver. `Order.delivery_*` stays populated, which matters because the webhook rebuilds the NOT NULL `BuyerOrder.delivery_address` from those four fields. The code and a real delivery address are mutually exclusive by construction: submitting an address clears the code, and removing the code at review sends the buyer back for an address. That is what stops a pickup buyer landing on a truck route. `_order_is_pickup(buyer_order)` is the single classifier; `_build_pickup_queue()` feeds the "Pickup" tab of the ops sidebar and `_build_delivery_queue()` filters those orders out. Handoff is recorded on `InventoryItem.picked_up_by_buyer_at` / `picked_up_by_buyer_by_id` (migration `39737a4ac282`) — pickups never produce a `DeliveryStop`, so that stamp is the only record the buyer took the item.
...
| `POST /admin/delivery/stop/<stop_id>/move` | `admin_delivery_move_stop` | Move one `pending` or `issue` stop onto a different shift+truck. Moves the row (BuyerOrder.delivery_stop is 1:1), resets it to pending, clears completion/POD/notification/load state, and keeps a failed attempt as a dated line in `notes`. `completed` stops are refused. A buyer who was already notified gets the rescheduled email. |
| `POST /admin/delivery/stops/move` | `admin_delivery_move_stops` | Same move for several stops at once (`stop_ids` CSV) — how ops takes one buyer, or a whole route, off a day that will not run. One rescheduled email per person, not per item. |
| `POST /admin/delivery/stops/notify` | `admin_delivery_notify_buyer_stops` | Delivery-scheduled email covering every listed stop (`stop_ids` CSV), which must all belong to one buyer. Backs the drawer's per-buyer Notify button; the per-stop route only covers one checkout. |
...
admin/ops_delivery_truck_detail.html  — Delivery truck drawer partial (Spec D1). Rows are buyers, not items: `_group_delivery_stops_for_display()` nests a buyer's items under one stop, with per-item tickboxes driving the whole-buyer move and a per-buyer Notify. Carries its own copy of the drawer CSS (it used to borrow the pickup partial's, which is only in the document if a pickup truck was opened first).
...
admin/warehouse_route_results.html  — Warehouse route browse item results (no layout). Same structure as warehouse_search_results.html but shows all items from shift sellers (no status filter); green unit chip if already placed. Top of partial renders a "📋 Photo Report" link (target=_blank) to admin_warehouse_route_photo_report when `shift_id` is in context.
admin/warehouse_route_photo_report.html — Standalone printable single-route photo report (does NOT extend admin_layout; own <head>, links style.css for CSS vars). Header (route label + date, item count, generated-at ET, Print + Back buttons) then the route's stops. Includes admin/_photo_report_styles.html and imports admin/_photo_report_stops.html.
admin/warehouse_route_photo_report_all.html — Standalone printable ALL-routes photo report (same head/styles as single-route), tuned for a dense at-a-glance audit. Header has an "All items / Not yet matched" segmented toggle (links to `?unmatched=1`); mode-aware heading + empty state. Includes admin/_photo_report_lightbox.html. Wide wrap (.rpr-wrap--wide, 1500px). Page header (route count, item count, generated-at ET, Print + Back), then a .rpr-routes-flow flex-wrap container of .rpr-route blocks: each is a compact route header (serif label + item count) + ONE packed 7-wide grid of all that route's items (render_route_packed). No per-stop headers and no amber notice (dropped for density; single-route report keeps both). Routes are sized to content and capped at 7 cards wide, so small routes pack onto shared rows. @media print keeps the packed flow (no per-route page break) but still avoids splitting a route or a card across pages. Includes _photo_report_styles.html, imports _photo_report_stops.html.
admin/_photo_report_styles.html — Shared <style> block for both photo reports (card size vars --rpr-card/--rpr-cardgap, grid, cards, headers, amber notice, all-routes .rpr-routes-flow packing rules, print rules). Relies on style.css CSS vars.
admin/_photo_report_stops.html — Shared Jinja macros: card(item, seller) (one photo card: item.original_photo_url — the seller's ORIGINAL non-AI photo, not the ai_enhanced_* cover — or #id placeholder box, + #id + 2-line-clamped title + seller, via url_for('uploaded_file')); render_stop(stop, number) + render_route_stops(route, show_notice=true) (single-route: one section per stop, ordered then NULL-order under "Unordered stops", optional amber "route not finalized" notice); render_route_packed(route) (all-routes: one grid of every item in stop order, columns pinned to min(total items, 7), no stop headers). Each card's img now carries a `data-photos` JSON attribute (item.original_photo_urls → served URLs) so the lightbox can page through all of an item's original photos.
admin/_photo_report_lightbox.html — Click-to-enlarge lightbox + CAROUSEL for both photo reports (include before </body>). Delegated click on `.rpr-zoom`; reads the card's `data-photos` list, opens at the clicked photo. Prev/next arrow buttons, ←/→ keys, clicking the image advances, `N / M` counter; single-photo items get `.single` (chrome hidden). Backdrop/Escape closes. Screen-only (hidden in print).
...
admin/rephoto.html             — Re-photography main page (extends admin_layout.html). Campaign banner ("started <date>"), autofocused debounced (300ms) search box fetching the search partial into #rp-results (stale-response guard), "+ Add an item that isn't listed" button (creates stub → opens capture modal in add mode). Header also has a "Match to Sellers" button → admin_rephoto_match_report. Includes rephoto_capture_modal.html.
admin/rephoto_report.html      — Rephoto MATCHING report (extends admin_layout.html). Clickable dense grid (auto-fill minmax(150px)) of rephotographed items showing `rephoto_photo_url` + #id + title; "Campus Swap backlog" / "All rephotographed" scope toggle. Clicking a card opens an in-page match modal: photo preview, editable Title, seller type-ahead (reuses /admin/warehouse/seller-search), an "original listing this replaces" <select> populated from /admin/warehouse/seller-items after a seller is picked, and L×W×H dimension inputs (flex row with min-width:0 to beat the global input{width:100%}). Save POSTs to admin_rephoto_match_save (CSRF via X-CSRFToken header, per rephoto.html pattern), then removes the card live and decrements the count. Modal footer also has backlog-disposition buttons (POST admin_rephoto_dispose): **Discard duplicate** + **Keep & list for Campus Swap** (shown for undisposed internal items), **Restore to backlog** (shown for discarded/kept items in scope=all), and **Revert to Campus Swap** (shown for matched real-seller items). Fields: a **Quantity** input (multi-unit stock) and a **Mattress size** dropdown (auto-fills L/W from MATTRESS_SIZES). Picking Campus Swap in the seller search relabels Save → "Revert to Campus Swap" and skips the storage requirement. Cards carry data-disposition / data-internal / data-mattress-size; scope=all cards show a Discarded/Kept badge.
...
### Rephoto Backlog Dispositions, Revert, Stock & Mattress Size
Layered onto the rephoto matching flow (`admin_rephoto_dispose`, `admin_rephoto_match_save`).

- **Backlog disposition (`rephoto_disposition`):** when a Campus-Swap-owned rephoto item isn't matched to a real seller, it's cleared from the backlog (`scope=internal`) as either `'discarded'` (junk/duplicate — also barred from the shop grid + teaser via a NULL-safe `rephoto_disposition.is_distinct_from('discarded')` filter) or `'kept'` (Campus Swap keeps & lists it — stays internal-owned, publishes to the shop when ready). Both leave the item visible under `scope=all` with a Discarded/Kept badge. `restore` clears back to NULL (a listed keeper is reverted `available`→`pending_valuation`).
- **Keep = list as Campus Swap:** `_publish_rephoto_if_ready` only requires `seller_id` present, so the internal account qualifies; a kept item auto-lists the moment it's AI-approved (`admin_ai_approve` calls the publish check). The shop has NO seller filter, so internal-owned items sell like any other. Payout reconciliation queues already exclude internal; the admin "payout owed" banner total was also fixed to exclude internal house sales.
- **Revert to Campus Swap (`_revert_rephoto_item_to_campus_swap`):** undoes a MATCH — sets seller back to the internal account, clears disposition (returns to backlog), reverts `available`→`pending_valuation` (pulls from shop), and restores any original it had replaced (clears that original's `replaced_by_item_id`, sets `ai_approved=True`). Reached via a "Revert to Campus Swap" button OR by picking Campus Swap in the match-save seller search (the old "Choose a real seller" hard block now reverts).
- **Multi-unit stock (Design B — clone into N rows):** `stock_quantity` entered in the Keep flow drives `_expand_stock_group(item)` (called from `_publish_rephoto_if_ready` on publish): clones the item into N one-unit rows sharing a fresh `stock_group_id`, copying listing/display/logistics fields (`COPY_FIELDS`, including `mattress_size`) + gallery photos. Each unit is a normal one-off item, so the sale/payout/delivery chain is untouched. The shop grid collapses each group to ONE representative card (lowest-id available unit) via a `NOT EXISTS` filter that preserves pagination; `stock_available_count` powers the "N available" badge; `_pick_available_stock_unit(item, cart_id)` selects a free unit in `cart_add` so concurrent buyers get distinct units. Chosen over a `quantity` column to avoid rewiring the money/delivery path (the `BuyerOrder.item_id` unique constraint, per-row `sold_at`/`payout_sent` scalars, and one-DeliveryStop-per-BuyerOrder all assume one unit per row).
- **Mattress size (`mattress_size`, `MATTRESS_SIZES`):** dropdown in the match modal autofills canonical L/W via `_apply_mattress_size_from_form` (validates the key; height/thickness left to the operator); carried through stock cloning. Product page shows nominal size + numeric dims when `mattress_size` is set — keyed off the field, NOT category. New `dim` Jinja filter formats a dimension without a trailing `.0` (`60.0` → `60`), used for both the mattress line and the regular dimensions line in `product.html`.
...
22. **Delivery helpers & warehouse origin** — `WAREHOUSE_DEFAULT_LAT='35.9030324'` / `WAREHOUSE_DEFAULT_LNG='-79.0709049'` (module constants, 515 S Greensboro St, Carrboro NC) are the default delivery origin used whenever the `warehouse_lat`/`warehouse_lng` AppSettings are unset. `_delivery_window(ref_date=None)` returns `{standard, flexible, friday, saturday, flex_end}` — deliveries run the upcoming Fri/Sat (Mon–Thu → this week, Fri–Sun → next week); used in the confirmation email, `item_success`, and `checkout_review`. `_send_delivery_scheduled_email(stop)` builds + sends one delivery-scheduled email and sets `notified_at` (caller commits); shared by `admin_delivery_notify_stop` (per-stop), `admin_delivery_notify_buyer_stops` (per-buyer) and `admin_shift_notify_buyers` (bulk). `_send_delivery_rescheduled_email(stops, old_date)` is its counterpart for a stop that moves after the buyer was told a date. `_group_stops_by_buyer_order(stops)` groups by checkout; `_group_stops_by_buyer(stops)` groups by person (email + address) and is what ops rows, buyer counts, and buyer-facing email all key on — a repeat buyer is one door. `_move_stops_to_route(stops, shift, truck)` is the one place stops change route; `_parse_move_target()` / `_check_stops_movable()` hold the guards (completed, mixed pickup truck, unique(shift, buyer_order)). `_email_photo_url(filename)` returns an absolute, email-safe image URL (see pattern #8).
