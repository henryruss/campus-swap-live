...
| `GET /admin/warehouse/rephoto/report` | `admin_rephoto_match_report` | Rephoto **matching** report. Clickable grid of rephotographed items (`_rephoto_reshot_item_ids`); `?scope=internal` (default — Campus-Swap-owned backlog, excludes rejected/sold) or `?scope=all`. Shows `item.rephoto_photo_url`. `_has_ops_access()`. |
