...
**Active spec:** (no formal spec) Rephoto matching follow-ons + Stock + Mattress size (built 2026-07-21/22)
**Overall status:** Specs #1–9 + Admin UI Redesign + all previous features in production. Buyer side now has a full cart + bundle flow: Spec D1 (Delivery Routes, 2026-05-29), Spec A (Delivery Fees / zone pricing + sales tax, 2026-06-14), and Spec B (Cart / Bundle & Save, 2026-06-18) are all built and in production. AI autofill runs through a background `_ai_queue` worker with bounded retries (`ai_retry_count`, hard stop at 3) and a startup requeue (2026-06-21). A shop + delivery ops pass (2026-06-22) hardened the buyer checkout (Google Maps address autocomplete + in-range confirmation, two-radio Standard/Flexible picker, concrete delivery date ranges, warehouse-origin fail-open default), replaced the cart-membership hold with a checkout-based hold, added buyer-notification tooling on the admin delivery side, and fixed email rendering (idempotent template, PNG logo, item photos). Warehouse Re-Photography (2026-07-08) adds a search-first guided three-shot capture flow for the UNC re-shoot campaign. Route Photo Report (2026-07-16) adds read-only, printable photo audit pages — one per route AND one covering every route on a single page. URL rule count is now 281. Item Dimensions (2026-07-17): optional L/W/H inches (`length_in`/`width_in`/`height_in`, `Numeric(5,1)` nullable, migration `74fd31ce2f07`) on add/edit item forms + buyer product page. Rephoto Matching (2026-07-17): matching-game tool at `/admin/warehouse/rephoto/report` — reassign Campus-Swap-owned rephotographed items to their real sellers and hide the seller's original listing (`replaced_by_item_id`, migration `454f7f6bc046`). Rephoto follow-ons + Stock + Mattress (2026-07-21/22): route-report "Not yet matched" toggle + lightbox carousel; backlog dispositions (discard/keep/restore) + revert-to-Campus-Swap; multi-unit stock (Design B — clone into N rows, "N available"); mattress size dropdown + dual shop display. **⚠️ PENDING ON RENDER: run `flask db upgrade`** — head is now `700635cb195e`, applies three stacked migrations in chain order: `46f98d884eeb` (rephoto_disposition) → `c2554b94906c` (stock_quantity + stock_group_id) → `700635cb195e` (mattress_size). Until then the shop/matching pages error on the missing columns.

---

## Features Built This Session (2026-07-21/22) — Rephoto follow-ons, Stock & Mattress

All in production code; **three migrations applied locally, PENDING `flask db upgrade` on Render** (chain: `46f98d884eeb` → `c2554b94906c` → `700635cb195e`). Each feature verified end-to-end via the real HTTP routes against the live local DB (throwaway items created + cleaned up); templates parse.

**1. Route Photo Report — "Not yet matched" toggle.** `/admin/warehouse/routes/photo-report` gained a segmented toggle. `_build_route_photo_report(shift, unmatched_only=False)`. "Not yet matched" keeps the per-route layout but shows only items NOT yet through a rephoto match — excludes BOTH the superseded original (`replaced_by_item_id` set) AND the replacement item (target of another row's `replaced_by_item_id`, via a `NOT IN replacement_target_ids` subquery). This corrected an earlier wrong criterion (a match removes the original but the replacement took its place, so the count looked unchanged). Routes/stops with nothing unmatched drop out.

**2. Route Photo Report — lightbox carousel.** `admin/_photo_report_lightbox.html` is now a multi-photo carousel (prev/next, ←/→ keys, image-click advances, `N/M` counter, single-photo hides chrome). New `InventoryItem.original_photo_urls` property (ordered original, non-`ai_enhanced_`, photo filenames); cards emit a `data-photos` JSON attribute. Verified with a headless-browser harness driving the actual JS.

**3. Rephoto backlog dispositions.** New column `rephoto_disposition` (NULL | 'discarded' | 'kept'), migration `46f98d884eeb`. New route `POST /admin/warehouse/rephoto/dispose/<item_id>` (`admin_rephoto_dispose`, `_has_ops_access()`): `discard` (junk — barred from shop), `keep` (Campus Swap keeps & lists; requires storage, dims optional; sets quantity/mattress), `restore` (undo → back to backlog), `revert` (undo a match). Backlog (scope=internal) excludes dispositioned items; scope=all shows a Discarded/Kept badge. Shop grid + teaser exclude `'discarded'`.

**4. Keep = list as Campus Swap.** `_publish_rephoto_if_ready` only needs `seller_id` present, so internal-owned items publish and sell (shop has no seller filter). Kept items auto-list on AI approval. Fixed the admin "payout owed" banner to exclude internal house sales (payout queues already did).

**5. Revert to Campus Swap.** `_revert_rephoto_item_to_campus_swap(item)` — seller→internal, disposition NULL (back to backlog), `available`→`pending_valuation`, restores the original it replaced (`replaced_by_item_id`=NULL, `ai_approved`=True). Reached via a "Revert to Campus Swap" button OR by picking Campus Swap in the seller search (the old "Choose a real seller" block now reverts). `admin_warehouse_seller_search` now returns `is_internal`.

**6. Multi-unit stock (Design B).** New columns `stock_quantity` (int, default 1) + `stock_group_id` (String 36, indexed), migration `c2554b94906c`. Quantity entered in the Keep flow → `_expand_stock_group` clones the item into N one-unit rows sharing a group id on publish (each a normal one-off item, so sale/payout/delivery are untouched). Shop grid collapses each group to one card (`NOT EXISTS` filter, pagination-safe) with an "N available" badge (`stock_available_count`); `_pick_available_stock_unit` gives concurrent buyers distinct units in `cart_add`. Chose Design B over a `quantity` column to avoid rewiring the money/delivery path — see DECISIONS.md.

**7. Mattress size.** New column `mattress_size` + `MATTRESS_SIZES` map (models.py) + `mattress_size_label` property, migration `700635cb195e`. Dropdown in the match modal autofills canonical L/W (`_apply_mattress_size_from_form`); carried through cloning. Product page shows nominal size + dims when set (keyed off `mattress_size`, not category). New `dim` Jinja filter drops trailing `.0`.
...
2. **Location picker added to the add-path details step (spec reversal, Henry 2026-07-09):** the spec's "storage stays NULL" decision is reversed. The details card now has an optional tap-through picker: "Select unit & spot" chip → bottom-sheet grid of active storage units (full units tappable but tagged "Full") → spatial 6-zone grid (back/middle/front × left/right) with "Skip — just the unit". `admin_rephoto_set_details` accepts optional `storage_location_id` + `storage_row` (validated via `_validate_storage_zone`); both stay NULL when not provided. `admin_rephoto_page` now passes `storage_locations` to the template.
3. **Subcategory picker added to the details step:** picking a category fetches `/api/subcategories/<id>`; when subs exist a pill row appears ("Pick a type:"), single-sub categories auto-select, none required. `admin_rephoto_set_details` accepts optional `subcategory_id` — silently NULLed unless it's a child of the chosen category (same rule as `edit_item`).
4. **Bug fix — `delete_photo` no longer bounces admins to ops:** the route had `if current_user.is_admin: redirect(admin_panel)` (which 302s to `/admin/ops`), so deleting a gallery photo from the edit-item view mid-edit dumped admins on the ops page. It now always redirects back to `edit_item` (its only caller). Tests: 41/41.
5. **Admin seller reassignment on the edit-item page:** `edit_item.html` gains an admin-only "Seller" card — shows the current seller (flags Campus Swap internal), live search (reuses `/admin/warehouse/seller-search`), picked seller shown as a "will be reassigned on save" chip with ✕ undo. `edit_item` POST applies `new_seller_id` for admins only (validated user id; ignored for sellers). No notification is sent on reassignment. Built for moving QC items off the internal account onto real sellers; the item appears on the new seller's dashboard automatically via `seller_id`.
6. **Admin post-save redirect on `edit_item`** now goes to `/admin/items` (was `admin_panel`, which 302s to `/admin/ops` — same bounce-to-ops class as the delete_photo bug; the old `#pending-items` anchor was dead anyway).
7. **Drawer delete returns JSON:** the items-tab detail drawer POSTs `/admin/item/<id>/delete` with `modal=1` and expects JSON, but the route always 302'd — the item deleted fine but the UI showed "Failed to delete. Try again." and the row lingered. `modal=1` now returns `{success: true}` (JSON error on failure); non-modal keeps flash+redirect. **Verified: no item-delete path (`admin_delete_item_direct`, `admin_quick_capture_delete`, `crew_quick_capture_delete`) sends any seller notification — by design.** Tests: 46/46.
8. **Bug fix — `/admin/items` full-screen white blocker on mobile portrait:** `items.html` carried an old inline-styled copy of `#item-detail-drawer` (`style="right:-500px"`, JS toggling `style.right`), while `style.css` has newer class-based rules for the same id that convert it to a bottom sheet at ≤640px with `right:0 !important`. On phones the `!important` beat the inline "closed" position and the inline `top:0;height:100vh` beat the sheet's `bottom:-100vh` — so the closed drawer rendered as an unclosable full-screen white panel (its ✕ set `style.right`, which can't out-rank `!important`); rotating past 640px "fixed" it. Reproduced and verified with Playwright at 390×844. Fix: converted items.html to the class-based pattern already used by `warehouse.html`/`storage_audit.html` (bare markup positioned by style.css, `.open` class toggling, guard watches `class` instead of `style`). Verified: no blocker on load, opens as bottom sheet on mobile / right drawer on desktop, closes, rotation round-trip clean. **Rule: never position `#item-detail-drawer` (or any element style.css owns) with inline styles.**
